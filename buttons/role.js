module.exports = {
    ...
    run: async (bot, interaction, parameters) => {
        ...
        const member = await interaction.guild.member.fetch(interaction.member.id)

        // if member has the role => remove role
        if (member.roles.cache.has(role.id)) {
            await member.roles.remove(role.id)
            return interaction.reply({ content: `Removed the role ${role.name} from you!`, ephemeral: true })
        }
        // if member does not have the role => add role
        else {
...
// ephemeral means that only the person who used the interaction is able to see the message. 